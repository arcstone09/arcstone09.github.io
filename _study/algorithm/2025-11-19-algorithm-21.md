...
위 GENEREIC-MST 프로시저의 임의의 시점에 그래프 $G_A = (V, A)$는 forest이며, 각각의 connected components 는 tree이다. 초기에 $A= \empty$ 일 때는 $V$개의 tree가 $G_A$에 존재하고, 매 iteration 마다 1씩 감소하여, $|V|-1$번의 iteration 후에는 1개의 tree (MST)만이 남게 된다. 
...
알고리즘의 수행시간은 $O(ElgE) = O(ElgV)$ $(\because |E| < |V|^2)$ 이다. (교재 633pg 참고)
...
이 때, light edge를 빠르게 구하기 위하여, tree $A$ 에 속하지 않는 vertices들은 min-priority queue $Q$에 $key$ attribute을 기준으로 저장된다. $v.key$는 tree $A$와 $v$를 연결하는 edge 중 minimum weight을 가지고 있고, 없다면 $\infin$ 가 저장된다.

알고리즘에 명시적으로 등장하지는 않지만 결국 프림 알고리즘은 MST의 root를 $r$로 했을 때, $A=\set {(v, v.\pi) : v \in V-\set{r}-Q}$ 로 키워나가는 셈이 된다. 알고리즘이 종료되는 시점에 $Q=\empty$ 이므로, $A=\set {(v, v.\pi) : v \in V-\set{r}}$  이 된다.
...
3. $\forall v \in Q$ if $v.\pi \neq NIL$, then $v.key < \infin$ and $v.key$ 는 $v$를 MST에 연결하는 light edge인 $(v, v.\pi)$ 의 weight을 가지고 있다.
