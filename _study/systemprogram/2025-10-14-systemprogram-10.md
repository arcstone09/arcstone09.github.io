...
📝 **ELF, PTE, fd, disk, RAM 관계**

프로그램 수행시, ELF, PTE, RAM 등의 유기적인 관계가 헷갈리는데 이를 정리해보자. `exec()` 를 통해 프로그램이 시작되면 커널이 ELF 파일은 open 하여 fd를 확보한다. (open 과정과 fd 관련된 내용은 unix file system 글 참조.) fd를 이용하여 inode를 통해 이 파일이 디스크 어느 블록에 있는지를 알게 된다. 

커널은 ELF의 각 segment를 가상 주소에 매핑해둔다. 아직 디스크에서 내용을 읽지는 않은 상태이다. 단, 커널은 ELF를 실행할 때 ELF 헤더와 program header는 먼저 읽고 시작 주소를 알 수 있다. CPU가 해당 시작 주소에서 시작하여 코드의 주소에 접근하면 아직 PTE는 비어있으므로, page fault가 발생한다. 그럼 커널이 실행되는데, PTE를 보고 이건 파일 기반 매핑이다라고 판단하여(파일 기반 매핑이 아닌 예는 스택 값을 디스크에서 저장하거나 디스크에 저장된 스택값을 읽어오는 경우를 생각할 수 있다.) 해당 가상 주소로부터 file offset을 계산하고 inode에서 디스크 블록 위치를 찾아 디스크에서 읽어서 RAM에 올린다. 그리고 PTE를 업데이트 한다. 

(VMA는 Virtual Memory Area로, 프로세스의 가상 주소 공간을 여러 구간으로 나누고 각 구간이 어떤 성질(코드인지, 데이터인지, 파일에서 온 것인지 등)을 가지는지를 설명하는 커널 내부 구조다; 프로그램 실행 중 page fault가 발생하면 커널은 해당 주소가 속한 VMA를 찾아 이 메모리가 파일에 대응되는 영역인지(예: ELF 코드) 아니면 단순히 새로 만들어야 하는 익명 영역인지(예: 스택, 힙)를 판단하고, 파일 기반이면 VMA에 저장된 offset 정보를 이용해 디스크에서 내용을 읽어오고 그렇지 않으면 빈 페이지를 할당한다, 즉 이 과정에서는 사용자 수준의 fd나 별도의 파일 핸들을 쓰지 않고 VMA에 기록된 정보만으로 처리된다.)
