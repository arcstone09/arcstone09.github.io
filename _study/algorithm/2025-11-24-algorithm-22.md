...
- Bellman-Ford : 각 edge를 $|V|-1$ 번 relax
...
**critiacal path** 란 dag에서 longest path를 의미한다. 일반적으로 longest path 문제는 NP-hard이지만, DAG에서는 acyclic 하므로 모든 edge들의 가중치를 (-1)배 한다음, shortest paths를 구하면 된다. 또는, INITIALIZE-SINGLE-SOURCE에서, $\infin$을 $-\infin$ 으로 바꾸고 RELAX에서 $>$을 $<$로 바꾸면 된다.
